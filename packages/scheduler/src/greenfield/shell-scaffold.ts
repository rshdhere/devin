...
function readmeForShell(title: string, stack: StackRuntime): string {
  const runHint =
    stack === "go"
      ? "go run ."
      : stack === "rust"
  ...
  return safeScaffoldReadme(title, stack, runHint);
}
...
export function greenfieldShellScaffoldFiles(opts: {
  ...
  const readme: ScaffoldFile = {
    path: "README.md",
    content: readmeForShell(opts.title, opts.stackRuntime),
  };
