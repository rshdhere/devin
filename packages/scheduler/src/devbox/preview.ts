...
export function buildDesktopScreenshotScript(
  ...
  return [
    ...
    'if [ -n "$B" ]; then',
    `"$B" --headless --disable-gpu --no-sandbox --window-size=1280,720 --hide-scrollbars --run-all-compositor-stages-before-draw --virtual-time-budget=8000 --screenshot='${safeOut}' '${safeUrl}' && exit 0`,
    "fi",
    'node "$SCRIPT"',
  ].join("\n");
}
