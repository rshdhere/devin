...
export function createTaskRouter(tasks: TaskService): Router {
  ...
  router.post("/:id/desktop/ensure", async (req, res) => {
    try {
      const upstream = await tasks.ensureDesktopComputer(req.params.id);
      res.status(upstream.status);
      res.type(upstream.headers.get("content-type") ?? "application/json");
      res.send(await upstream.text());
    } catch (error) {
      sendError(res, 502, error, "desktop ensure failed");
    }
  });

  router.get("/:id/desktop-vnc", async (req, res) => {
    try {
      await tasks.proxyDesktopVNCPageHttp(req.params.id, res);
    } catch (error) {
      sendError(res, 502, error, "desktop vnc failed");
    }
  });
